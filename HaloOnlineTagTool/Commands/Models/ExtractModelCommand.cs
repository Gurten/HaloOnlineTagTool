                ...
                using (var objFile = new StreamWriter(File.Open(fileName, FileMode.Create, FileAccess.Write)))
                {
                    ...
                    if (variant != null)
                    {
                        ...
                        foreach (var region in variant.Regions)
                        {
                            ...
                            for (var i = 0; i < meshCount; i++)
                            {
                                ...
                                objExtractor.ExtractMesh(meshReader, RenderModelUtils.getMatForMesh(renderModel, renderModel.Geometry.Meshes[meshIndex + i]), vertexCompressor, resourceStream);
                            }
                        }
                    }
                    else
                    {
                        ...
                        foreach (var mesh in renderModel.Geometry.Meshes)
                        {
                            ...
                            objExtractor.ExtractMesh(meshReader, RenderModelUtils.getMatForMesh(renderModel, mesh), vertexCompressor, resourceStream);
                        }
                    }
                    objExtractor.Finish();
                }
